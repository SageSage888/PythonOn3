...
async def get_film_planeta():
    async with aiohttp.ClientSession() as session:
        url = "https://planetakino.ua/movies/"
        async with session.get(url) as response:
            text = await response.read()
            results = BeautifulSoup(text, "lxml").find_all(
                'div', class_="content__section movies__section")
            for result in results:
                return Film(cinema="Planeta",
                            title=result.find(
                                'div', class_="movie-block__mobile-name").text.replace('\n', '').strip(),
                            age=result.find(
                                'div', class_="movie-block__age").text,
                            session=result.find('div', class_="movie-block__mobile-text-date").text.replace('\n',
                                                                                                            ' ').strip(),
                            ) """
